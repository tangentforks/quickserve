...
export class API2Client {
    box; // remote hostname; empty → relative URLs (browser mode)
    ver;
    uid;
    epw;
    sid;
    ...
    _url(ep, fmt_) {
        ...
        const ee = ep.split('?');
        if (!ee[0].includes(fmt_))
            ee[0] += fmt_;
        ep = ee.join('?');
        const path = `/api/${ep}`;
        if (this.box)
            return `https://${this.box}/${this.ver}${path}`;
        if (this.ver)
            return `/${this.ver}${path}`;
        return path;
    }
    ...
    login(d) {
        ...
        if (d.uid)
            ...
            throw new Error('Expected d.uid in API2Client.login(d)!');
        if (!d.pwd)
            throw new Error('Expected d.pwd in API2Client.login(d)!');
